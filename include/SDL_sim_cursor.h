...
extern void SDL_SIM_RenderCursorScaled(SDL_Texture *texture, float scale, int logical_w, int logical_h);
...
{
    SDL_SIM_RenderCursorScaled(texture, 1.0, -1, -1);
}

void SDL_SIM_RenderCursorScaled(SDL_Texture *texture, float scale, int logical_w, int logical_h)
{
    SDL_SIM_Mouse *mouse = SDL_SIM_GetMouse();
    SDL_Cursor *cursor;
    SDL_Rect mouse_dest;
    ...
    {
        // Same code as cursor_get_pos()
        float fx, fy;
        SDL_RenderWindowToLogical(mouse->renderer, x, y, &fx, &fy);
        x = fx < 0 ? 0 : (fx >= logical_w ? logical_w - 1 : (int)fx);
        y = fy < 0 ? 0 : (fy >= logical_h ? logical_h - 1 : (int)fy);
    }
